...
function loadEnvironments() {
    ...
    if(environments[NODE_ENV] != undefined){
        ...
        if (fs.existsSync(envFilePath)) {
            winston.info(`[config] [loadEnvironments] loading: ${envFilePath}`)
            dotenv.config({ path: envFilePath })
        }
    } 
    else if(fs.existsSync(defaultFilePath)){   
    //     //try take .env
    winston.info(`[config] [loadEnvironments] loading: ${defaultFilePath}`)
         dotenv.config({ path: envFilePath })
     }
    else{
        winston.info(`[config] [loadEnvironments] could not load any .env config including .env.${Object.keys(environments).join(' | ')}`)
